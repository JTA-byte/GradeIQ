/**
 * Card vision analysis using the Claude API.
 * Takes up to 10 labeled card photos (full front/back plus optional corner
 * close-ups) in a single call and returns a structured condition assessment
 * (centering, surface, edges, corners) that feeds the ROI engine.
 */
...
const SYSTEM_PROMPT = `You are an expert Pokémon TCG card grader with deep knowledge of PSA, CGC, and BGS grading standards. You analyze up to 10 labeled photos of a single card -- a full front shot, a full back shot, and up to 8 optional close-ups of each corner/quadrant (e.g. "Front Top-Left", "Back Bottom-Right") -- and provide one objective condition assessment for the card as a whole.
